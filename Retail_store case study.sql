CREATE DATABASE RETAIL_DB;  -- Creating the database

USE RETAIL_DB;              -- Accessing the database

CREATE TABLE SALES_FACT (   -- Creating a table named sales_fact
...
CREATE TABLE ORDER_PRODUCT_MAPPING (     -- Creating a table named order_product_mapping
	ORDER_ID INT,
...
CREATE TABLE PRODUCT_INFO (             -- Creating a table named product_info
...
CREATE TABLE CUSTOMER_INFO (           -- Creating a table named customer_info
...
CREATE TABLE BRANCH_DETAILS (           -- Creating a table named branch_details
...
ALTER TABLE SALES_FACT                 -- Adding a foreign key constraint
ADD CONSTRAINT ORDER_ID_FN
	FOREIGN KEY(ORDER_ID) REFERENCES ORDER_PRODUCT_MAPPING(ORDER_ID);
    
ALTER TABLE ORDER_PRODUCT_MAPPING      -- Adding a foreign key constraint
...
/* Before inserting values into the table it is important to remember that we have defined foreign. Hence we need to first insert records
   in the dimension tables before touching the fact tables. */ 

INSERT INTO PRODUCT_INFO               -- Inserting values into Product_info table from excel
VALUES
...
INSERT INTO CUSTOMER_INFO             -- Inserting values into customer_info table from excel data
VALUES
...
INSERT INTO ORDER_PRODUCT_MAPPING             -- Inserting records into order_product_mapping table from excel data
VALUES
(1000,'7713',6),
(1001,'3690',3),
(1001,'4925',8),
...
INSERT INTO SALES_FACT               -- Inserting records into sales_fact table from excel data
VALUES 
...
-- 1. Number of transactions by GENDER
     -- Men
     -- Women
	SELECT GENDER, COUNT(opm.ORDER_ID) AS ORDER_COUNT FROM ORDER_PRODUCT_MAPPING opm
	LEFT JOIN SALES_FACT sf
	ON opm.ORDER_ID = sf.ORDER_ID
	LEFT JOIN CUSTOMER_INFO ci
	ON sf.CUSTOMER_ID = ci.CUSTOMER_ID
	GROUP BY GENDER;

-- 2. Fing the number of customers who have placed an order from more than one store
	SELECT COUNT(CUSTOMER_ID) AS CUST_COUNT FROM
	(SELECT CUSTOMER_ID FROM ORDER_PRODUCT_MAPPING OPM
	LEFT JOIN SALES_FACT SF
	ON OPM.ORDER_ID = SF.ORDER_ID
	GROUP BY CUSTOMER_ID
	HAVING COUNT(DISTINCT STORE_ID)>1) AS all_orders;

-- 3. Find the city with highest number of orders
	SELECT CITY,COUNT(OPM.ORDER_ID) AS ORDER_COUNT FROM ORDER_PRODUCT_MAPPING OPM
	LEFT JOIN SALES_FACT SF
	ON OPM.ORDER_ID = SF.ORDER_ID
	LEFT JOIN BRANCH_DETAILS BD
	ON SF.STORE_ID = BD.STORE_ID
	GROUP BY CITY
	ORDER BY COUNT(OPM.ORDER_ID) DESC
	LIMIT 1;

-- 4. Find the area with highest number of sales
	SELECT AREA, SUM(FINAL_PRICE) AS SALES FROM 
	(SELECT OPM.ORDER_ID,OPM.PRODUCT_ID,QUANTITY_ORDERED,AREA,SALE_PRICE, (QUANTITY_ORDERED * SALE_PRICE) AS FINAL_PRICE FROM ORDER_PRODUCT_MAPPING OPM
	LEFT JOIN PRODUCT_INFO PF
	ON OPM.PRODUCT_ID = PF.PRODUCT_ID
	LEFT JOIN SALES_FACT SF
	ON OPM.ORDER_ID = SF.ORDER_ID
	LEFT JOIN BRANCH_DETAILS BD
	ON SF.STORE_ID = BD.STORE_ID) AS FINAL
	GROUP BY AREA
	ORDER BY SUM(FINAL_PRICE) DESC
	LIMIT 1;

-- 5. Which state has the highest number of customers
	SELECT STATE, COUNT(SF.CUSTOMER_ID) AS CUST_COUNT 
	FROM SALES_FACT SF
	LEFT JOIN CUSTOMER_INFO CI
	ON SF.CUSTOMER_ID = CI.CUSTOMER_ID
	LEFT JOIN BRANCH_DETAILS BD
	ON SF.STORE_ID = BD.STORE_ID
	GROUP BY STATE
	ORDER BY COUNT(SF.CUSTOMER_ID) DESC
	LIMIT 1;